...
字体程序用于定义功能定义，在极少数情况下，还用于定义指令。
函数 0 从控制值表中读取一个值，对该值进行四舍五入，并将四舍五入后的值写回控制值表。 它需要两个参数。 调用函数时，栈顶的项应该是从中读取值的控制值表地址。 堆栈上的第二项应该是值将被写入的控制值表位置。
...
如下所示的函数 1 可用于控制衬线宽度。 控制值表条目 3 包含纽约衬线宽度的适当值。 它有两个参数，参考点编号和要进行 MIRP 处理的点。
...
<table>
  ...
  <tr>
    <td>MIRP[00100]</td>
    <td>移动点 34，直到它与 rp0 的距离为控制值表条目 3 中的值（衬线宽度）。</td>
  </tr>
  <tr>
    <td>ENDF[]</td>
    <td>结束当前函数定义。</td>
...
功能1可用于指示大写字母I，如下所示。下面给出纽约的扩展控制值表。
...
第 6 点被“点缀”，以便它可以作为未来指令的参考点。

<table>
  <tr>
    <td rowspan="10">
      <img src = "./images/I-1.gif" />
    </td>
    <td>PUSHB[000]</td>
    <td>将一个字节压入堆栈。</td>
  </tr>
...
从点 6 到点 19，词干宽度由“MIRPing”控制。

<table>
  <tr>
    <td rowspan="10">
      <img src = "./images/I-2.gif" />
    </td>
    <td>PUSHB[001]</td>
    <td>将两个字节压入堆栈。</td>
  </tr>
  ...
  <tr>
    <td rowspan="7" >MIRP[11101]</td>
    <td>移动点 19，直到它与点 6 的水平距离为控制值表位置 4 中的值（大写字干宽度）。</td>
  </tr>
  <tr>
    <td>将 rp1 设置为 rp0。</td>
  </tr>
  <tr>
    <td>将 rp0 更改为第 19 点。</td>
  </tr>
</table>

使用 LOOPCALL[] 命令进行的连续函数调用用于控制四个衬线。
...
<table>
  ...
  <tr>
    <td>SCVTCI</td>
    <td>设置控制值切入为 70/64</td>
  </tr>
</table>

下一组指令用于为字形中每个 em 的像素数小于 50 的情况建立替代控制值切入。在这种情况下，默认设置为 128/64ths，即两个像素。

<table>
  <tr>
    <td>MPPEM[]</td>
    <td>获取当前字形中每个 em 的像素数。</td>
  </tr>
  <tr>
    <td>50</td>
    <td>每 em 值的像素。</td>
  ...
  <tr>
    <td>GT[]</td>
    <td>将堆栈顶部的数字 (50) 与其下方的数字（每个 em 的像素数）进行比较。 如果 ppem > 50，则将 TRUE 压入堆栈。 否则，将推送 false。</td>
  </tr>
  <tr>
    <td>IF[]</td>
    <td>如果比较结果为 TRUE，则执行 IF 语句的主体。</td>
  </tr>
  <tr>
    <td>PUSHB[]</td>
    <td>将一个字节压入堆栈。</td>
  ...
  <tr>
    <td>EIF[]</td>
    <td> if 语句结束。(end if)</td>
  </tr>
</table>

接下来，在暗区中建立了一些点。 第一步是将区域指针设置为指向区域 0，模糊区域。
...
矢量设置为 y 轴，以便可以控制垂直关系。


<table>
  <tr>
    <td>SVTCA[0]</td>
    <td>设置自由度和投影矢量指向 y 轴。</td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td rowspan="10">MIAP[1]</td>
    <td>将暮光点 0 设置为控制值表条目 14（上升）的圆形高度。</td>
  </tr>
  <tr>
    <td>将 rp0 和 rp1 设置为点 0。</td>
  </tr>
...
<table>
  ...
  <tr>
    <td rowspan="10">1</td>
    <td>将暮光点 1 设置为控制值表条目 1（上升）的圆形高度。</td>
  </tr>
  <tr>
    <td>将 rp0 和 rp1 设置为点 0。</td>
  </tr>
...
<table>
  ...
  <tr>
    <td>MIAP[0]</td>
    <td>将微光点 2 设置为控制值表条目编号 0 中指定的未舍入高度（上限高度）。</td>
  </tr>
</table>

上限高度在上升和基线之间进行插值，以保持其相对于这两个舍入值的位置。 获取内插点的坐标，并将其值写回控制值表位置 0。
...
<table>
  ...
  <tr>
    <td>SRP1[]</td>
    <td>将参考点 1 设置为点 1（基线高度）。请注意，rp2 仍位于上升高度点 0。</td>
  </tr>
  <tr>
    <td>PUSHB[000]</td>
    <td>将一个字节压入堆栈。</td>
...
调用函数 0 以读取、舍入然后写回新插入的控制值表条目 0。由于现在调用的函数不访问字形区域中的任何点，因此无需将区域指针重置为区域 1。默认值 区域指针的值将为每个字形程序自动重新设置。
...
<table>
  ...
  <tr>
    <td>CALL[]</td>
    <td>使用 FDED 0 读取、舍入和写入新内插的控制值表条目。</td>
  </tr>
</table>

## 检查结果

字体是否成功的最佳测试是查看预期用途的典型示例。 查看您打算打印的示例。 如果希望字体在 72 dpi 设备上用作屏幕字体，请查看屏幕上 10 点和 12 点的样本测试。 但是，其他测试在隔离特定问题方面可能很有用。
瀑布图是一行以每 em 减少一个像素的大小重复的文本，使重量跳跃更加明显。 图 25 中显示了带有屏幕字体的瀑布。图 25 显示了高分辨率打印机上的相同文本。

**图 24** 在 72 dpi 下具有 9 到 24 点日内瓦的瀑布

![图24](./images/fig3-24.gif)

**图25** 6到24点日内瓦的瀑布

![图25](./images/fig3-25.gif)

比较连续大小的字体的粗体和普通变体的文本可以确保它们很容易区分。

**图26**

![图26](./images/fig3-26.gif)

间距测试可让您了解字母在上下文中的外观。 检查 H 和 O 之间的每个大写字母。这使您可以看到字形在平面和圆形字形旁边的外观。 对于小写字母，使用 n 和 o。

**图 27** 12 点纽约的间距测试

![图27](./images/fig3-27.gif)

另一个有用的测试是查看大写字母和小写字母。

**图 28** 大写字母后跟小写字形

![图28](./images/fig3-28.gif)

Pangrams 允许您在少量文本中查看整个字母表。

**图29** 一个Pangrams

![图29](./images/fig3-29.gif)

检查上升和下降的对齐以及比较具有相似特征的字符也很重要。

**图 30** 相关字形集

![图30](./images/fig3-30.gif)