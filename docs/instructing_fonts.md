...
要创建控制值表条目 4，即大写词干宽度，有必要查看整个字体的关键字形中词干的宽度，并找到一个所有人都能接受的值。 一种可能的方法是对相关值进行平均。

一旦确定了值，就应该在控制值表中输入必要的控制值表条目。

正如几何分析所建议的以及随后的说明将揭示的那样，H 的说明将需要多个控制值表条目。 它们如表 1 所示。
...
H 的网格拟合指令必须引用构成字形描述的轮廓点。 图 11 显示了大写 H 轮廓以及定义该轮廓的曲线上和曲线外点。 此字形的说明如下。 每条说明都附有对其所做工作的解释，如果相关，还附有对其影响或使用的要点的说明。
...
应用于 New York H 的第一组指令控制字形在其前进宽度内的位置。 一种稍微更复杂的控制字形放置的方法是将公共左侧方位聚类，并从原点使用指定的控制值表条目对它们进行 MIRP[]。
...
下一组指令用于控制字形左侧的空间。 这是通过 MIRP[] 指令控制左茎上的关键点到原点的距离来实现的。 参考点 rp0 被改变以准备下一条指令。
